...
#SBATCH --ntasks=1
#SBATCH --array=0-0
#SBATCH --output=<BASE_DIR>/<USERID>/logs/mono_ali/bucket3/combine_info_%A_%a.out
#SBATCH --error=<BASE_DIR>/<USERID>/logs/mono_ali/bucket3/combine_info_%A_%a.err
...
if [[ "$DATASET_NAME" == "speech_v1.0" ]]; then
    echo "Processing ${DATASET_NAME} ${lang_dir}"
    ...
    results_dir="${root}/lhotse_data_concat_target_aligned/${DATASET_NAME}${lang_dir}"

else
    echo "Processing ${DATASET_NAME} bucket${bucket}"
    ...
    results_dir="${root}/lhotse_data_concat_target_aligned/${DATASET_NAME}/bucket${bucket}"
fi

mkdir -p "${results_dir}"
mkdir -p "${root}/logs/mono_ali/bucket${bucket}"
