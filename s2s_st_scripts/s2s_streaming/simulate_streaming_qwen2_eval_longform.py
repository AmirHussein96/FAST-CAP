...
# audio buffer size
AUDIO_BUFFER_SIZE = 70 * input_audio_chunk_size_samples 
...
class DuplexModel:
    ...
    def reset_inference_state(self):
        ...
        """
        Resets the state of the model for the same inference session. This is
        called when the audio is done or the text is done.
        """
        # Counter for tracking inference steps (chunks)
        self.counter = 0
        # --- State for Autoregressive Generation ---
    ...
    def execute(self, audio_in_chunk, audio_done, txt_done, predefined_spk, reset):
       ...
       # audio buffer sliding window
        # if self.audio_in_buffer.shape[1] >= AUDIO_BUFFER_SIZE:
        #     self.audio_in_buffer = self.audio_in_buffer[:, -AUDIO_BUFFER_SIZE:]
        #     min_audio_samples_for_next_inference = AUDIO_BUFFER_SIZE 
        
        # speaker prompt sliding window
        if not predefined_spk:
            if self.audio_in_buffer.shape[1] <= prompt_length:
                prompt = self.audio_in_buffer
                self.spk_embedding = self.update_prompt(prompt, USER_SAMPLE_RATE)
            elif self.audio_in_buffer.shape[1] > prompt_length:
        ...
        buffer_len_tensor = torch.tensor([self.audio_in_buffer.shape[1]], device=DEVICE)
        ...
        audio_to_encode = self.audio_in_buffer.to(torch.float32)
...
if __name__ == '__main__':
    ...
    for i,cut in tqdm(enumerate(cuts), desc="Processing cuts"):
        ...
        NUM_CHUNKS_TEST_AUDIO = int(y.shape[1]/(input_audio_chunk_size_samples))*1.3
        ...
        with torch.inference_mode():
            for i, audio_in_chunk in enumerate(audio_signal_chunks):
                ...
                if audio_out.size > 0:
                    ...
                    partial_generated_audio.append(audio_out)
                # if EOS is detected or the audio reached the maximum 20s then reset
                if (audio_done.all() or txt_done.all()) or (chunk_count > 0 and chunk_count % 225 == 0):
                    print("✅ EOS detected.")
                    audio_done = torch.zeros([1], dtype=torch.bool, device=DEVICE)
                    txt_done = torch.zeros([1], dtype=torch.bool, device=DEVICE)
                    dm.reset_inference_state()
                    if partial_generated_audio:
                        partial_generated_audio = np.concatenate(partial_generated_audio, axis=1)
                        partial_generated_audio = torch.from_numpy(partial_generated_audio).to(device=DEVICE, dtype=torch.float32)
                        partial_generated_audio_len = torch.tensor([partial_generated_audio.shape[1]], dtype=torch.long).to(DEVICE)
                        asr_hyps.append(asr_bleu.update(
                            name=cut.id,
                            refs=[target_text],
                            pred_audio=resample(partial_generated_audio, AGENT_SAMPLE_RATE, USER_SAMPLE_RATE),
                            pred_audio_lens=(partial_generated_audio_len / AGENT_SAMPLE_RATE * USER_SAMPLE_RATE).to(torch.long),
                        )[0])
                        partial_generated_audio = []
                        chunk_count = 0
        ...
        asr_hyps = " ".join(asr_hyps)
        if tts_final_audio.size > 0:
            import soundfile as sf
            from numpy import pad
            output_audio_path = f"{args.output_dir}/{cut.id}.wav"
        ...
        entry = {
            ...
            "speech_pred_transcribed": asr_hyps,
